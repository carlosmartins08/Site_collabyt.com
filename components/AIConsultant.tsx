...
const initialMessages: ChatMessage[] = [
  {
    role: 'model',
    text: 'Olá! Sou a IA da Collaby.t. Posso fazer uma análise técnica preliminar da sua ideia agora mesmo. O que você quer construir?'
  }
];
...
const AIConsultant: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (response.status === 429) {
        updateLastModelMessage('Limite diário atingido. Vamos continuar amanhã ou fale com um especialista.');
        return;
      }
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    if (!leadForm.consent) {
      setLeadError('Você precisa aceitar o consentimento.');
      return;
    }
    ...
    try {
      ...
      if (!response.leadToken) {
        throw new Error('Token de lead não retornado');
      }
      ...
      setLeadError('Não foi possível validar seu contato. Tente novamente.');
    } finally {
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4 pointer-events-none">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600 leading-snug">
                    Posso fazer uma análise técnica rápida do seu projeto agora.
                  </p>
                </div>
              </div>
            </motion.div>
        ...
        >
          ...
          <div className="relative z-10 flex items-center gap-2">
            ...
            {!isOpen && <span className="hidden md:inline font-bold pr-1">Diagnóstico IA</span>}
          </div>
        </motion.button>
      </div>
          ...
          >
            ...
            <div className="bg-white/5 p-4 flex items-center justify-between border-b border-white/5">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setMessages([{ role: 'model', text: 'Reiniciando... Olá! Como posso ajudar?' }])}
                  ...
                  <form onSubmit={handleLeadSubmit} className="mt-4 space-y-3">
                    ...
                    <label className="flex items-start gap-2 text-xs text-slate-400">
                      <input
                        ...
                        required
                      />
                      <span>Autorizo o contato da Collaby.t para este diagnóstico.</span>
                    </label>
                ...
                <div className="p-4 bg-brandBlack/50 backdrop-blur-md border-t border-white/5">
                  ...
                  <div className="text-center mt-3 flex justify-center items-center gap-1.5 opacity-60">
                    ...
                    <span className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">
                      Lead validado. Limite diário aplicado.
                    </span>
                  </div>
                </div>
              </>

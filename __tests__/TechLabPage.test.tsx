...
const renderTechLabPage = (locale: 'pt-br' | 'en') =>
  render(
    <HelmetProvider>
      <MemoryRouter initialEntries={[`/${locale}/tech-lab`]}>
        <LanguageProvider>
          <TechLabPage locale={locale} />
        </LanguageProvider>
      </MemoryRouter>
    </HelmetProvider>
  );

test('renders tech lab page in pt-br with CTA and layout', () => {
  renderTechLabPage('pt-br');

  expect(
    screen.getByRole('heading', { level: 1, name: /Tech/i })
  ).toBeInTheDocument();
...
test('renders tech lab page in en with CTA and layout', () => {
  renderTechLabPage('en');

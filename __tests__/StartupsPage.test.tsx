...
const renderStartupsPage = (locale: 'pt-br' | 'en') =>
  render(
    <HelmetProvider>
      <MemoryRouter initialEntries={[`/${locale}/startups`]}>
        <LanguageProvider>
          <StartupsPage locale={locale} />
        </LanguageProvider>
      </MemoryRouter>
    </HelmetProvider>
  );

test('renders startups page in pt-br with CTA and layout', () => {
  renderStartupsPage('pt-br');

  expect(
    screen.getByRole('heading', { level: 1, name: /Startups/i })
  ).toBeInTheDocument();
...
test('renders startups page in en with CTA and layout', () => {
  renderStartupsPage('en');

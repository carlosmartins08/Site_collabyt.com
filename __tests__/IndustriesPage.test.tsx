...
const renderIndustriesPage = (locale: 'pt-br' | 'en') =>
  render(
    <HelmetProvider>
      <MemoryRouter initialEntries={[`/${locale}/industries`]}>
        <LanguageProvider>
          <IndustriesPage locale={locale} />
        </LanguageProvider>
      </MemoryRouter>
    </HelmetProvider>
  );

test('renders industries page in pt-br with CTA and layout', () => {
  renderIndustriesPage('pt-br');

  expect(
    screen.getByRole('heading', { level: 1, name: /Industrias/i })
  ).toBeInTheDocument();
...
test('renders industries page in en with CTA and layout', () => {
  renderIndustriesPage('en');

  expect(
    screen.getByRole('heading', { level: 1, name: /Industries/i })
  ).toBeInTheDocument();
